...
String sql1 = "select count(*) total from cs_notice " + str;
...
String sql2 = "select b.* from( "
		...
		+ " select unq ,n_title, to_char(n_wdate,'yyyy-mm-dd') n_wdate, hits from cs_notice " + str +" order by unq desc) a) b "
...
<%
				while(rs2.next()){		
					...
					String title = rs2.getString("n_title");
					String wdate = rs2.getString("n_wdate");
...
%>				
		...
		<%
			if((pg_sno-1) > 0){
		%>
		<a href="notice.jsp?page_no=<%=pg_sno-1 %>" class="n_btn02">[before]</a>	
		<%
			}	
			for(int i = pg_sno; i <= pg_eno; i++)	{
		%>		
			<a href="notice.jsp?page_no=<%=i %>" class="n_btn02"><%=i %></a>
		<% 
			}		
			if(pg_eno != lastpage){
		%>
		<a href="notice.jsp?page_no=<%=pg_eno+1 %>" class="n_btn02">[next]</a>
		<%
			}
		%>			
